...
static void get_descriptor(void) {
	...
	if (setup_packet->bmrequesttype == 0x80) {
		...
		if (descriptorType == DEVICE_DESCRIPTOR) {
			// request_handled = 1;
			 code_ptr = (codePtr) &deviceDescriptor;
...
void _test02 () {
    if (USBIF) {
        ...
        USBIF = 0;
        SOFIF = 0;// many times we couldn't count it
        TRNIF = 0;// occur 1 time
        ACTVIF = 0;// occur 1 time
        IDLEIF = 0;// occur 2 times
        URSTIF = 0;// occur 4 times
    }
}
